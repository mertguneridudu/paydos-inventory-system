...
<body id="kt_body" class="header-fixed header-tablet-and-mobile-fixed toolbar-enabled toolbar-fixed" style="
				...
				<div class="content d-flex flex-column flex-column-fluid" id="kt_content">
					<div class="post d-flex flex-column-fluid" id="kt_post">
									...
									<table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_category_table">
										<thead>
											<tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
												...
												<th class="min-w-250px">GSM Package Content Name</th>
                      ...
                      @foreach ( $gsmPackageContent as $contentItem)
											<tr id="rem_{{ $contentItem->gsm_package_content_id }}">
												<td class="text-gray-800 text-hover-primary fs-5 fw-bolder mb-1">
													{{ $contentItem->gsm_package_name }}

												</td>
												<td class="text-gray-800 text-hover-primary fs-5 fw-bolder mb-1">
                        {{ $contentItem->gsm_package_content_name }}
												...
												<td>
                                                    <div class="d-flex justify-content-end flex-shrink-0">
                                                        <a href="{{ url('/gsm-package-content/' . $contentItem->gsm_package_content_id . '/edit') }}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
													...
													<td class="text-end">
														@csrf
														<a href="javascript:();" onclick="removeGsmPackageContentItem({{$contentItem->gsm_package_content_id}})" data-id="{{ $contentItem->gsm_package_content_id }}"  class="btn btn-danger px-3 btremove">Remove</a>
													</td>
	...
	<script>
		function removeGsmPackageContentItem(id) {
			if (confirm('Are you sure you want to remove this item?')) {
				$.ajax({
					url: '/gsm-package-content/remove',
					type: 'post',
					data: {
						"_token": "{{ csrf_token() }}",
						gsm_package_content_id: id
					},
