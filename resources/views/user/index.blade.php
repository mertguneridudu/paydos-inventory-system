...
<body id="kt_body" class="header-fixed header-tablet-and-mobile-fixed toolbar-enabled toolbar-fixed" style="
					...
					<div class="post d-flex flex-column-fluid" id="kt_post">
									...
									<table class="table table-bordered align-middle table-row-dashed fs-6 gy-5" id="datatable">
										<thead>
											<tr class="text-start text-muted fw-bolder fs-7 text-uppercase gs-0">
												...
												<th class="min-w-100px">Inventory Info</th>
												@if ( Auth::user()->yetki == 1)
                        ...
                        <th class="text-end min-w-100px">Remove</th>
                                ...
                                @endif
												...
												<th class="text-end min-w-50px">Custodian</th>
											</tr>
										</thead>
                                            ...
                                            @foreach ($users as $item)
                                                <tr class="remove" id="rem_{{$item->id}}">
                                                    <td class="d-flex align-items-center">
                                                        ...
                                                        @endif
															<td>
																	<span class="badge py-3 px-4 fs-7 badge-light-success">Has Inventory</span>
															</td>
																@if ( Auth::user()->yetki == 1)
																<td>
																...
																<td class="text-end">
																		@csrf
																		<a href="javascript:();" onclick="RemoveKullanici({{$item->id}})" data-id="{{ $item->id }}"  class="btn btn-danger px-3 btremove">Remove</a>
																</td>
                                @else
                                ...
                                @endif
															...
															<td class="text-end">
																<a href="{{ url('/user/assignment/' . $item->id) }}" class="btn btn-warning px-3 btremove">Custodian PDF</a>
															</td>
                                                </tr>
                                            @endforeach
										</tbody>
    ...
    <script>
        function RemoveKullanici(id) {
					...
					if (confirm('Do you want to remove this user? Removing a user will make all user inventories inactive!')) {
						$.ajax({
