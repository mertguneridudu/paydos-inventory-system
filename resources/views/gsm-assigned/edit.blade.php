			...
			<div class="content d-flex flex-column flex-column-fluid" id="kt_content">
				...
				<div class="post d-flex flex-column-fluid" id="kt_post">
					...
					<div id="kt_content_container" class="container-xxl">
						...
						<form action="{{ url('gsm-assigned/' .$gsm_table[0]->gsm_assigned_id)}}" method="POST" class="form d-flex flex-column flex-lg-row">
							@csrf
							@method("PATCH")
											...
											<div class="card card-flush py-4">
												...
												<div class="card-header">
													<div class="card-title">
														<h2>Edit GSM Number Assignment</h2>
													</div>
												</div>
														...
														<div class="row">
															...
															<div class="col-lg-6 fv-row">
																...
																<select class="form-select mb-2" name="gsm_assigned_user" data-control="select2"
																	...
																	@foreach ($users as $userItem)
																		<option value="{{ $userItem->id }}" @if ($userItem->id == $gsm_table[0]->gsm_assigned_user )
																			selected
																				@endif>{{ $userItem->name }} {{ $userItem->surname }}</option>
																	@endforeach
																</select>
															...
															<div class="col-lg-6 fv-row">
																...
																<input autocomplete="off" type="text" name="gsm_number" class="form-control mb-2"
																   placeholder="Number" value="{{ $gsm_table[0]->gsm_number }}" />
															</div>
														...
														<div class="row">
															...
															<div class="col-lg-6 fv-row">
																...
																<select class="form-select mb-2" name="gsm_assigned_package_id" data-control="select2"
																	...
																	<option></option>
																	@foreach ($gsm_package as $packageItem)
																		<option value="{{ $packageItem->gsm_package_id }}" @if ($packageItem->gsm_package_id == $gsm_table[0]->gsm_assigned_package_id )
																			selected
																				@endif>{{ $packageItem->gsm_package_name }}</option>
																	@endforeach
																</select>
															...
															<div class="col-lg-6 fv-row">
																...
																<select class="form-select mb-2" name="gsm_assigned_package_content_id" data-control="select2"
																	...
																	@foreach ($gsmPackageContent as $contentItem)
																		<option value="{{ $contentItem->gsm_package_content_id }}" @if ($contentItem->gsm_package_content_id == $gsm_table[0]->gsm_assigned_package_content_id )
																			selected
																				@endif>{{ $contentItem->gsm_package_content_name }}</option>
																	@endforeach
																</select>
...
<script>
	$(document).ready(function () {
		...
		var selectedGsmPackageId = $('select[name="gsm_assigned_package_id"]').val();
